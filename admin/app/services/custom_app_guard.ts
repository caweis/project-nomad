...
export function getStorageRoot(): string {
  // NOMAD_STORAGE_PATH is declared `optional()` in start/env.ts (no validation beyond
  // "string"), so reading it straight from process.env is behaviour-identical to going
  // through the Adonis env service — `env.get('NOMAD_STORAGE_PATH', default)` returns
  // exactly `process.env.NOMAD_STORAGE_PATH ?? default`. Reading process.env directly
  // keeps this pure module free of the Adonis runtime so it can be exercised under
  // `node --experimental-strip-types` (the local test contract; Adonis env can't boot
  // without MySQL/Redis present). No validation is lost.
  const configured = process.env.NOMAD_STORAGE_PATH || '/opt/project-nomad/storage'
  return normalize(configured).replace(/\/+$/, '')
}
