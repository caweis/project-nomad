 ...
 * (`storage/NOMAD.md` under the app root, which the container bind-mounts from
 * the host data directory), so an end user can also edit it directly on disk.
 * The file is created lazily on the first save — a missing or empty file simply
 * means no custom prompt is injected.
 */
export class NomadMdService {
  static STORAGE_PATH = 'storage/NOMAD.md'

  private get filePath(): string {
    return join(process.cwd(), NomadMdService.STORAGE_PATH)
  }
