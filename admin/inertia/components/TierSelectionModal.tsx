...
const TierSelectionModal: React.FC<TierSelectionModalProps> = ({
  ...
  // Get all resources for a tier (including inherited resources). Null-safe so
  // the hooks below can run before the `!category` early return (hook order
  // must stay constant while category transitions null → non-null on open).
  const getAllResourcesForTier = (tier: SpecTier): SpecResource[] => {
    if (!category) return []
    return resolveTierResources(tier, category.tiers)
  }

  // Pre-compute the selected tier's resources outside the JSX. Empty when
  // nothing is selected.
  ...
  // Embedding disk-cost estimate for the selected tier (RFC #883 §1).
  ...
  // the other consumers, so a fresh cached copy is reused).
  ...
  // Cancel returns to the tier modal as-is; Proceed runs the original
  // onSelectTier path.
  const [guardrailVerdict, setGuardrailVerdict] = useState<GuardrailVerdict | null>(null)

  // Free bytes on the root filesystem; 0 means "unknown", which the guardrail
  // helper treats as "skip the relative-disk check". (Our fork has no
  // getPrimaryDiskInfo helper — derive from fsSize like the server-side
  // preflight does: prefer the root mount, else the largest available.)
  const freeBytes = useMemo<number>(() => {
    const fsSize = systemInfo?.fsSize
    if (!fsSize?.length) return 0
    const root = fsSize.find((f) => f.mount === '/')
    if (root) return Math.max(0, root.available)
    return Math.max(0, ...fsSize.map((f) => f.available))
  }, [systemInfo])
              ...
              <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-lg bg-surface-primary shadow-xl transition-all">
                ...
                <div className="p-6">
                  ...
                  {/* Embedding-cost preview — visible whenever a tier is
                      selected. Projects how much extra disk the AI Assistant
                      needs for these files on top of the raw downloads. */}
