...
The upstream project runs on Linux with NVIDIA GPUs. The earlier Mac forks got partway there — snfettig wired up native Ollama, proximasan patched the admin for Apple Silicon. This repo takes that work, finishes the install pipeline so it doesn't break on a fresh Mac, rebuilds the admin Docker image from current source so it actually runs on `arm64`, gives Ollama direct access to the Metal GPU instead of stuffing it inside a Linux VM, and wraps everything in a `nomad` command that handles install, upgrades, repair, and lifecycle from the Terminal you already have open.
