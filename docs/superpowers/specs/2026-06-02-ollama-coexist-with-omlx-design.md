...
| D12 | Delivery: a **`nomad ai-clients`** helper (working name) + an **install-flow offer** + **broadening the existing AI help page** (`/docs/mac-ai-assistant`, retitled beyond "AI Assistant", URL kept) + man entry. The helper is **guided and does the work**: it asks one question — *where will you use the app (this Mac / another device on your network / away from home)* — and acts accordingly (loopback + local URL; or flip LAN-expose + print `http://nomad.local:11434`; or print tunnel steps), then hands you the recipe (official download link + exact config), optionally running an app's *own* official installer (Zed brew cask, Claude Code npm). No `.env`/`launchctl` editing by the user; it never re-hosts a third-party binary. | Discoverable, install-grade, configure-scope; license-clean (Agent! binary is author-reserved). | Docs only; nomad hosting/serving the binary. |
