...
| D4 | Bind **mode-aware + configurable**: omlx mode defaults **loopback**; LAN-expose is **opt-in and flipped *for* the user by `nomad ai-clients`** in one step (restarts the agent + prints `http://nomad.local:11434`; the choice persists in `.env` — **never hand-edited**, no `launchctl`). `ollama` mode is `0.0.0.0` (admin reach; already LAN-exposed). | Least-exposure default **and** a simple, no-config-editing opt-in for the remote-LAN-GUI case. | `0.0.0.0` default in omlx mode; making the user edit a `.env`/plist. |
...
| D12 | Delivery is the **admin web GUI** (per Chris — *not* the CLI): a **"Use from other devices / Connect an app"** section on the AI settings page — RAM-advised **user-defined** model menu (reusing admin's existing model management + pull path), a one-tap **LAN-expose toggle**, live **connection URLs** (`localhost` + `nomad.local`), and recommended-app **download links + paste-able configs**. Host-side actions run via the **host-command bridge** (D14). The broadened AI help page is the read-only companion. A thin `nomad` subcommand may *back* the bridge action, but it is **not** the user surface; never re-hosts a third-party binary. | GUI is the simplest surface — no terminal, no config files; reuses proven admin patterns. | CLI as the primary surface; `.env`/`launchctl` editing; hosting the binary. |
| D13 | Install integration: the installer prints a **one-line pointer to the admin GUI** ("set up local AI apps in the admin → Settings → AI"), skippable. | "It's for the install too" — discoverable without doing app setup in the fragile install path. | Running app setup inside the install script. |
| D14 | The LAN-expose (and any host-side) action goes through the **existing host-command bridge** with a **new allow-listed command** (e.g. `ai-lan-expose on\|off`), mirroring the `upgrade-ollama`/`reset-ollama` buttons. The Docker admin never touches the host directly; the bridge **allow-list is the security boundary**. | Reuses a proven pattern; keeps the container↔host trust boundary intact. | Admin shelling to the host directly; an unbounded/parameter-injectable host command. |
...
**Layer 2:**
- **Admin GUI** (`admin/inertia/pages/settings/…` + a controller method): a "Use from other devices / Connect an app" section on the AI settings page — model menu (reuse `ollama_service` + the existing pull path; RAM from `system_service.getSystemInfo`), the **LAN-expose toggle** (a `HostCommandButton`-style control), live connection URLs (`localhost` + `nomad.local`, backend-aware), and app download links + paste-able configs.
- **Host-command bridge**: add `ai-lan-expose on|off` to the allow-list (single-source `admin/constants/host_commands.ts`) + a host-side handler in `nomad` that flips the general-Ollama bind (loopback↔`0.0.0.0`) and restarts the agent; admin POSTs via the bridge and polls the result — same pattern as `upgrade-ollama`. For an *untrusted* network the GUI shows tunnel guidance instead (no exposure).
- Install pointer (D13): one skippable line directing to the admin GUI section.
...
- This adds real **admin code** (a settings section + controller + the new host-command + allow-list entry) — *not* a pure-docs change. The app binary itself stays external (user-installed).
...
- **Remote-LAN GUI (via the admin GUI toggle):** flipping **"Use from other devices" ON** in the admin (→ host-command bridge → bind flip + restart, no manual host edits) makes the user's Ollama models reachable at `nomad.local:11434` from another LAN device; OFF → not LAN-reachable. oMLX is reachable via `nomad.local:11436` regardless. All clients coexist with the admin + local clients. Bridge allow-list rejects anything but `ai-lan-expose on|off`.
