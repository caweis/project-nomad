...
1. **[BLOCKER — RAG FIXED 2026-06-03, `64c5a9a`] Ollama runner.** Brew ollama *formula* on macOS 26 is runner-less → all GGUF inference 500s ("llama-server binary not found"). LIVE-PROVEN fix: the official Ollama.app *binary* (`/Applications/Ollama.app/Contents/Resources/ollama`) carries a working runtime-fetched runner (embedded a vector in 0.3s; the formula 500'd). Shipped `_resolve_ollama_bin` (prefer the app binary, fall back to PATH; `NOMAD_OLLAMA_BIN` override) + routed `step_ollama_embed` (:11435) and `step_ollama_native` (:11434) through it + `_ollama_runner_present` reports present for the app binary. **VERIFIED on the mini:** `nomad upgrade`'s embed probe now prints "✓ embed Ollama inference OK (RAG embeddings working)"; embed agent runs the app binary; `:11435` and `:11434` both return real vectors. RAG is alive.
   - **REMAINING (the `:11434` ownership decision + reset hardening):** even after quitting the Ollama.app, `step_ollama_native`'s reset still ends with "could not free :11434" and the holder PID changes between runs → something (likely nomad's own `com.projectnomad.ollama` KeepAlive racing the kill — `bootout` is `|| true`, silent-fail) keeps respawning a *working* ollama on :11434. The daemon is healthy; the reset just can't cleanly tear-down-and-rebind an already-serving port, so `nomad upgrade` exits non-zero with a scary `die ":11434 must be free"`. DECIDED **C (coexist, don't fight)** — shipped `d0f808a`: in `step_ollama_native`, after the bootout, if :11434 is held by a process whose command runs the app binary (`*/Ollama.app/*` — the GUI's own server OR our respawned agent, both healthy with a working runner), log "coexisting" and `return 0`, skipping a second daemon; only a non-app orphan (e.g. the runner-less formula) is offered for reclaim, and a failed/declined kill warns + returns 0 — the reset **never `die`s over :11434** again. :11434 free → nomad bootstraps its own app-binary agent as before. Confirm on the mini: `nomad upgrade` → expect "general Ollama already serving :11434 off the app binary … coexisting" + clean exit. NOTE: with C, `ai-lan-expose` (Phase 2) can't flip the bind when the app GUI owns :11434 — revisit if/when that toggle is built. Follow-up: auto-install the cask app on a fresh install when absent.
...
- **NEXT BUILD — Optional AI-tools framework (DECIDED 2026-06-03, Chris scoped it).** Make nomad-served AI tools first-class OPTIONAL components, not baked-in. Chris's four requirements: (1) **install-time choice** — the installer asks whether to add each tool; (2) **FAQ/help page** documents them; (3) **install/remove after the fact via BOTH the `nomad` CLI AND the admin web GUI**; (4) **all man pages updated** (incl. a new `nomad-<cmd>.1`). Design: gate each optional service behind a **compose profile** enabled by an `.env` flag (e.g. `NOMAD_TOOLS=…` → `COMPOSE_PROFILES`); a `nomad` subcommand (e.g. `nomad tools <name> on|off`) flips the flag + `dc up/down`s the service; the admin GUI toggles via the **host-command bridge** (single-source allow-list `admin/constants/host_commands.ts` + `run_cmd` case in `nomad` + `test-host-command-allowlist.sh`, same pattern as `upgrade-ollama` / the planned `ai-lan-expose`). DRY — design the optional-tool mechanism ONCE; all tools use it. First two tools: **Open WebUI** (chat, `:3000`) and **OpenHands** (code, `:3001`). ⚠ Open WebUI is currently an UNCONDITIONAL compose service (`c888d82`/`5182020`); this feature **converts it to profile-gated/optional** (don't leave it always-on for everyone). Spec → plan → build FRESH (this session hit context limit).
- **OpenHands — the "code" tool within the framework above (DECIDED 2026-06-03, Chris).** Use case: autonomous AI coding from any LAN browser, offline ("apocalypse coding"). Realize it the NOMAD way — a managed service the appliance *serves* (like Open WebUI, shipped `c888d82`, and Kiwix), NOT a per-device app (Agent! and Zed both tried + rejected). Integration design to spec: (a) OpenHands container (`docker.all-hands.dev/all-hands-ai/openhands`) — note it **spawns a per-session runtime sandbox container**, so it needs the Docker socket (same posture as admin) + the runtime image pre-pulled for offline use; (b) **PORT 3001** — `:3000` is now Open WebUI; (c) LLM → local Ollama (LiteLLM `ollama/<model>` or OpenAI-compat, base `http://host.docker.internal:11434`) default `qwen2.5-coder:14b`; (d) persistent workspace + settings volume; (e) Apps-page tile + help-page mention; (f) **honest caveat** — autonomous agents want big models; a 14B local model is limited vs cloud (state it in UI/docs); (g) resource-heavy on a 32 GB mini alongside oMLX/Ollama — document/guard. Spec → plan → build in a FRESH session (this one hit context limit). Chat client decided = **Open WebUI** (shipped, compose service `c888d82`, `:3000`). CLI coding alt documented = **aider**.
